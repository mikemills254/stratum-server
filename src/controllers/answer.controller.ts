...
export class AnswerController {
    ...
    async handleUpdateAnswerText(req: Request, res: Response): Promise<Response> {
        try {
            const user = req.user;
            if (!user) return res.status(401).json({ message: "Unauthorized", success: false });
    ...
    async handleAnnotateAnswer(req: Request, res: Response): Promise<Response> {
        try {
            const user = req.user;
            if (!user) return res.status(401).json({ message: "Unauthorized", success: false });
            ...
            const { id } = req.params as { id: string };
            const { text } = req.body;
            if (!text?.trim()) return res.status(400).json({ message: "Annotation text is required", success: false });
